...
const ProfileScreen = ({ navigation }) => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          setProfilePicture(userData.profilePicture || ''); // Fetch base64 string
        } else {
  ...
  const pickImage = async () => {
    try {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        allowsEditing: true,
        aspect: [1, 1],
        quality: 0.5, //Reduce image quality to 50% for smaller size (to account for Firestore limits)
        base64: true, 
      });
  
      if (!result.canceled) { //Making sure image selection was not canceled
  ...
  const handleImageUpload = async () => {
    ...
    try {
      // Update the Firestore document with the base64 image string
      const userDocRef = doc(firestore, 'users', auth.currentUser.uid);
  ...
  return (
    ...
    >
      ...
      <View style={styles.profileSection}>
        ...
        >
          <Text style={styles.rowText}>Upload Profile Picture</Text>
        </TouchableOpacity>
